...
let cellElement = document.querySelector('tbody')
let tdQ = cellElement.children[0].childElementCount
for (let j = 0; j < cellElement.childElementCount; j++) {
  ///////////////////

  let randomColor1 = colorRandomizer()
  let randomColor2 = colorRandomizer()

  let gradientArray = new Gradient()
    .setColorGradient(randomColor1, randomColor2)
    .setMidpoint(5)
    .getColors()
  ...
  for (let i = 1; i < tdQ; i++) {
    ...
    cellElement.children[j].children[0].bgColor = randomColor1
    cellElement.children[j].children[i].bgColor = gradientArray[i - 1]
  }
}
